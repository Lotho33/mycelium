...
// Deliberately not built on _dangerPost: every other danger-zone action ends
// with a toast on the same dashboard, but this one wipes the admin identity
// that dashboard session depends on — success has to leave the tab on
// /setup, not sitting on a now-nonsensical authenticated page.
...
// ─── Side drawer + pages ─────────────────────────────────────────────────────
const _pages = ['overview','plugins','devices','users','vpn','settings','logs','download'];
const _pageTitles = {
    overview: 'Home', plugins: 'Plugin', devices: 'Dispositivi', users: 'Utenti',
    vpn: 'Rete', settings: 'Impostazioni',
...
// /admin/core/resources via updateCoreRes), Extractor e Redis
...
// ─── Utenti (profili Pileus, condivisi tra tutti i dispositivi) ──────────────
...
// findPileusProfile re-reads the current list rather than caching it, so a
// rename always starts from the server's actual current values — this box
// only has profile_id/name in its dataset-free innerHTML, nothing to read
// back client-side.
