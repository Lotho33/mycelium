...
func (m *LuaPluginManager) runTasks(p *LuaPlugin) {
	...
	// Delega a MarkTaskRan così il guard "stato = error → non registrare" vale
	// per lo scheduler cron esattamente come per il trigger manuale.
	markRan := MarkTaskRan

	shouldRunNow := func(t LuaManifestTask) bool {
		// cron == "" segna un task manual-only: compare comunque nella
		// dashboard (con un pulsante "Esegui") ma non deve MAI auto-
		// partire, né al boot né a intervalli — prima di questo, l'unico
		// modo per registrare un task manuale-nella-dashboard era dargli
		// un cron finto lunghissimo (es. "@every 87600h", 10 anni), che
		// però NON impediva la partenza automatica al boot: shouldRunNow
		// trattava "nessun record di ultima esecuzione" come "scaduto",
		// quindi un task manuale-solo-dashboard ripartiva da solo a ogni
		// riavvio del server se non era mai stato lanciato manualmente
		// prima — con un task di manutenzione pesante che riparte da solo
		// a ogni restart questo era un problema concreto, non teorico.
		if t.Cron == "" {
			return false
		}
		...
		val, err := managers.Redis.Get(ctx, taskLastRunKey(p.Manifest.ID, t.Function))
		if err != nil || val == "" {
			return true
		}
		var lastRun int64
	...
	run := func(t LuaManifestTask) {
		// Only a `running` plugin (started + all required settings filled)
		// schedules tasks. Re-checked here on every tick, not once at
		// scheduler setup, so start/stop and config changes take effect
		...
		// già un ritorno normale via err) prima non era recuperato: run()
		// gira in una goroutine nuda (go run(t)), quindi un panic non
		// recuperato scavalca TUTTI i defer sopra e crasha l'intero
		// processo — bgTaskSem/p.taskMu restavano acquisiti per sempre
		// anche se il crash non ci fosse stato, bloccando ogni task futuro
		// di OGNI plugin. Caso reale: crash ricorrenti del dev server durante
		// sync pesanti coincidevano esattamente con questo pattern (nessun
		// altro recover() esiste nel percorso di esecuzione dei task Lua).
		defer func() {
			if r := recover(); r != nil {
		...
		if err != nil {
			...
			markRan(p.Manifest.ID, t.Function)
		}
	...
	for _, task := range p.Manifest.Tasks {
		...
		t := task
		_, err := c.AddFunc(t.Cron, func() { run(t) })
		if err != nil {
