...
func setLuaPluginState(w http.ResponseWriter, r *http.Request) {
	...
	case "start":
		...
		// Senza questo, il primo sync di un plugin appena configurato non
		// partiva mai finché non si riavviava l'intero processo — vedi il
		// commento su RunDueTasksNow. Fire-and-forget: non blocca la risposta.
		engine.LuaPlugins.RunDueTasksNow(pluginID)
	case "stop":
