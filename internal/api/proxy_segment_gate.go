...
// noteChallengeIfAny logs that an upstream responded with an interactive
// verification page (advertised via the `Cf-Mitigated: challenge` response
// header) — operational visibility only (a CDN is blocking the proxy's
// fetches), no dashboard/admin surface reacts to this. domain = registrable
// domain of rawURL; egressLabel is the egress profile name (or "direct").
