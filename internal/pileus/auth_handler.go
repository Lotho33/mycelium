...
func (h *AuthHandler) CreateProfile(ctx context.Context, req *gen.CreateProfileRequest) (*gen.ProfileResponse, error) {
	...
	if err := insertProfile(deviceID, profileID, req.Name, req.AvatarUrl, req.IsChild, prefs); err != nil {
		return nil, status.Error(codes.Internal, "db error: "+err.Error())
	}
...
func (h *AuthHandler) UpdateProfile(ctx context.Context, req *gen.UpdateProfileRequest) (*gen.ProfileResponse, error) {
	...
	if err := updateProfile(req.ProfileId, name, avatarURL, req.IsChild); err != nil {
		return nil, status.Error(codes.Internal, "db error: "+err.Error())
	}
...
func insertProfile(deviceID, profileID, name, avatarURL string, isChild bool, prefsJSON string) error {
	_, err := managers.DB.Exec(
		`INSERT INTO pileus_profiles(profile_id, device_id, name, avatar_url, is_child, preferences, created_at)
		 VALUES(?,?,?,?,?,?,CURRENT_TIMESTAMP)`,
		profileID, deviceID, name, avatarURL, isChild, prefsJSON,
	)
	return err
}
...
func listProfiles() ([]*gen.ProfileResponse, error) {
	rows, err := managers.DB.Query(
		`SELECT profile_id, name, avatar_url, is_child, preferences
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ProfileId, &p.Name, &p.AvatarUrl, &p.IsChild, &p.PreferencesJson); err != nil {
			return nil, err
		}
		out = append(out, p)
	}
...
func getProfileRow(profileID string) (*profileRow, error) {
	...
	err := managers.DB.QueryRow(
		`SELECT name, avatar_url, is_child, preferences
		 FROM pileus_profiles WHERE profile_id=?`,
		profileID,
	).Scan(&row.name, &row.avatarURL, &row.isChild, &row.preferences)
	if err != nil {
		return nil, err
	}
	return row, nil
...
func updateProfile(profileID, name, avatarURL string, isChild bool) error {
	_, err := managers.DB.Exec(
		`UPDATE pileus_profiles
		 SET name=?, avatar_url=?, is_child=?
		 WHERE profile_id=?`,
		name, avatarURL, isChild, profileID,
	)
	return err
}
