...
func (h *MediaHandler) ResolveStream(req *gen.ResolveRequest, stream gen.MediaPipeline_ResolveStreamServer) error {
	...
	if err != nil {
		...
		// The upstream asked for interactive verification (the SDK sniff
		// prefixes the error with "CHALLENGE:<domain>"). Give Pileus a clear,
		// actionable message instead of a generic internal error.
		if dom, ok := challengeDomainFromErr(err.Error()); ok {
			return status.Errorf(codes.FailedPrecondition,
				"Verifica di sicurezza richiesta per %s. Risolvila dalla dashboard: Impostazioni → VPN → Verifiche in sospeso.", dom)
		}
	...
	if useAniSkip || useIntroDb {
		...
		if !fromCache {
			...
			// Fire-and-forget: playback starts now without skip markers for this
			// resolve. The result is cached so a future resolve of the same
			// episode (replay/resume) can pick it up instantly from cache instead
			// of blocking again.
	...
	if isHLS {
		...
		if perr := prebufferHLS(ctx, req.PluginId, rawURL, headers, isLive, sendProgress); perr != nil {
			// Only an interactive-verification hit on the playlist comes back
			// here (any other pre-buffer hiccup is swallowed as best-effort).
			// Fail the RPC now with the same actionable message the resolve
			// path uses, instead of returning a {result} the player can't play.
			if dom, ok := challengeDomainFromErr(perr.Error()); ok {
				return status.Errorf(codes.FailedPrecondition,
					"Verifica di sicurezza richiesta per %s. Risolvila dalla dashboard: Impostazioni → VPN → Verifiche in sospeso.", dom)
			}
		}
...
// challengeDomainFromErr pulls "<domain>" out of a resolve error the SDK
// prefixed with "CHALLENGE:<domain> …" when the browser service hit an
// interactive verification it couldn't complete.
func challengeDomainFromErr(msg string) (string, bool) {
	const p = "CHALLENGE:"
